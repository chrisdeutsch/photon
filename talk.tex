...
\note[itemize]{
	...
	\begin{itemize}
		\item photonuclear absorption: excitation of a nucleus -- decay with the emission of a neutron/gamma or fission
		\item Delbrück scattering: pair produced electron pair at a nucleus annihilates again
		\item nuclear scattering: Compton/Thomson scattering at nucleus, Mößbauer effect
	\end{itemize}
}
...
\begin{frame}{Photoelectric effect -- shell structure}
	\begin{columns}
		...
		\begin{itemize}
			\setlength\itemsep{1.5em}
			\item $\sigma_\mathrm{photo}$ depends on the energetically available electron shells
...
\begin{frame}{Photoelectric effect -- dependencies}
	\begin{itemize}
		\setlength\itemsep{1.5em}
		\item non-relativistic case incl.\ correction ($E_\mathrm{K} < E_\gamma \ll m_\mathrm{e}c^2$)
		\begin{align*}
		...
		\item relativistic case ($E_\gamma \gg m_\mathrm{e}c^2$)
		\begin{align*}
...
\begin{frame}{Compton scattering -- diff.\ cross section}
	\begin{columns}
		...
		\begin{itemize}
			...
			\begin{align*}
				P(E_\gamma, \theta_\gamma) := \frac{E_\gamma^\prime(E_\gamma, \theta_\gamma)}{E_\gamma}
			\end{align*}
		\end{itemize}
	...
	\begin{align*}
	\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega_\gamma} = \frac{r_\mathrm{e}^2}{2} \cdot P(E_\gamma, \theta_\gamma)^2 \bigg[ P(E_\gamma, \theta_\gamma) + \frac{1}{P(E_\gamma, \theta_\gamma)} + \cos^2\theta_\gamma - 1 \bigg]
	\end{align*}
\end{frame}
...
\begin{frame}{Compton scattering -- diff.\ cross section}
	\begin{columns}
		...
		\begin{itemize}
			...
			\begin{align*}
				\frac{\mathrm{d} \sigma}{\mathrm{d} T_\mathrm{e}} = \left(\frac{\mathrm{d}T_\mathrm{e}}{\mathrm{d}\cos\theta_\gamma}\right)^{-1} \cdot 2 \pi \frac{\mathrm{d}\sigma}{\mathrm{d}\Omega_\gamma}
			\end{align*}
		\end{itemize}
...
\begin{frame}{Pair production -- cross section}
	\begin{itemize}
		...
		\begin{align*}
			\sigma_\mathrm{pair} &\approx \frac{7}{9} \frac{1}{X_0}
			\begin{array}[t]{@{}c@{}}
...
\note[itemize]{
	\item ultimately the energy deposition works via charged particle interaction
	\item both parts can be split into different components
}
...
\begin{frame}{Detector concepts -- example from HEP}
	\centering
	\textbf{ALICE Photon Spectrometer (PHOS)}
	\begin{columns}
		\column[c]{0.6\textwidth}
		\begin{center}
			PbWO$_\text{4}$ scintillators with APD-readout
...
\note[itemize]{
	...
	\item ALICE Photon Spectrometer -- measure photons from heavy ion collisions
	\item active material PbWO$_\text{4}$: 18 cm long ($20 X_0$) with 2.2 cm square section ($1 R_\mathrm{M}$)
	\item allows for photons up to $10 \, \mathrm{GeV}$
	\item segmentation with independent readouts (avalanche photodiode) -- position resolution
	\item for accurate energy measurement: particles need to be contained
}
...
\begin{frame}{Rayleigh scattering}
	\begin{itemize}
		\setlength\itemsep{1em}
		\item coherent scattering at the electrons of an atom ($\lambda \gg d$)
		\item no excitation or ionization
	\end{itemize}
\end{frame}

\begin{frame}{Momentum conservation in pair production}
	\begin{align*}
	&k = p_1 + p_2 \rightarrow k^2 = (p_1 + p_2)^2 = 0 \\
	&0 = 2 m_\mathrm{e}^2 + 2 (E_1 E_2 - \vec{p}_1 \cdot \vec{p}_2) \geq 2 m_\mathrm{e}^2
	\end{align*}
